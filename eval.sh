...
#
# 常用覆盖示例：
#   CKPT_DIR=ckpt_stt_filtered NUM_PARALLEL=2 GPUS=0,1 ./eval.sh
...
SAVE_PATH="${SAVE_PATH:-sim_data/eval/stt_train}"    # 评估结果保存目录
EXP_CONFIG="${EXP_CONFIG:-habitat-lab/habitat/config/benchmark/nav/track/track_infer_stt.yaml}"

CKPT_DIR="${CKPT_DIR:-ckpt_stt_filtered}"      # 默认从这里找最新训练权重
CKPT="${CKPT:-}"                               # 手动指定权重路径；非空时优先使用
SAVE_VIDEO="${SAVE_VIDEO:-1}"                  # 是否保存评估视频

export DINOV3_MODEL_PATH="${DINOV3_MODEL_PATH:-/data/hdt/newtrackvla/weights/dinov3}"
