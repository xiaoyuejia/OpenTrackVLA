...
main_output=/data/hdt/ntv_data/sim_data/keyboard_collect_inverse_replay_m40_m8_speed100_gpu7
runtime=/data/hdt/ntv_data/sim_data/unreal_env_workers/gpu6
...
run_batch_maps_except_korean_palace() {
  ...
  for env_dir in "$source_root/$batch"/seed_*/*; do
    [[ -d "$env_dir" ]] || continue
    map=${env_dir##*/}
    [[ "$map" == *KoreanPalace* || "$map" == *Map_ChemicalPlant_1* ]] && continue
...
export MPLCONFIGDIR=/tmp/matplotlib-replay-gpu6-12h

run_batch_maps_except_korean_palace new5 "$main_output"
run_batch 'new11/seed_100/UnrealTrack-Greek_Island-ContinuousColor-v0' "$main_output" || true
run_batch_maps_except_korean_palace new5 "$main_output"
run_batch 'new11/seed_100/UnrealTrack-Greek_Island-ContinuousColor-v0' "$main_output" || true
