#!/usr/bin/env python3
"""Collect RobotDog episodes into the single-agent sim_data layout."""
...
ROBOTDOG_INFO_SUFFIX = "_robotdog_info.json"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Group RobotDog recordings by scene and assign consecutive episode IDs."
    )
...
def main() -> int:
    ...
    for info_path in sorted(input_root.rglob(f"*{ROBOTDOG_INFO_SUFFIX}")):
        stem = info_path.name[: -len(ROBOTDOG_INFO_SUFFIX)]
        video_path = info_path.with_name(f"{stem}_robotdog.mp4")
    ...
    if not episodes_by_scene:
        raise RuntimeError(f"No RobotDog episodes found under {input_root}")
