#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Habitat 原始单 Agent在线规划器与闭环评估实现。

整体功能：
- ``evaluate_agent`` 遍历 Habitat 数据集 episode，维护历史帧并执行闭环动作。
- ``GTBBoxAgent`` 加载 OpenTrackVLA checkpoint 或 Hugging Face 权重，完成视觉编码、
  bbox 条件构造、路点预测、动作转换和视频可视化。
- 保存每条 episode 的指标、帧信息和可选视频，供 Habitat 指标脚本汇总。

主要输入输出：
- 输入由 ``eval.py`` 提供：Habitat config、数据集切片和保存目录。
- 模型权重通常由环境变量 ``CKPT`` 指定。
- 输出为 Habitat 评估 JSON、视频及轨迹可视化。

本文件只服务 Habitat 单 Agent评估；UnrealZoo 双 Agent使用
...
try:
    ...
    from tools.cache_gridpool import VisionFeatureCacher, VisionCacheConfig, grid_pool_tokens
...
def evaluate_agent(config, dataset_split, save_path) -> None:
    ...
    with habitat.TrackEnv(
        ...
        for _ in trange(num_episodes):
            ...
            # 主循环：每一步获取观测、计算动作、执行动作、记录信息，并检查跟随状态和距离
...
class GTBBoxAgent(AgentConfig):
    ...
    def _resolve_planner_ckpt_once(self):
        ...
        ckpt_path = None
        try:
            ckpt_dir = '/data/hdt/ntv_data/ckpt/ckpts_qwen4'
        ...
        if ckpt_path:
            ...
            return
        default_hf = os.path.join(REPO_ROOT, 'open_trackvla_hf')
    ...
    def _download_hf_model(self, repo_id: str) -> Optional[str]:
        if snapshot_download is None:
            ...
            return None
        cache_root = os.path.join(REPO_ROOT, 'hf_downloads')
    ...
    # 将RGB帧编码为视觉特征tokens，供跨模态时空规划器使用
    ...
    def _planner_action(self, rgb_frame_np: np.ndarray, instruction: Optional[str]) -> Optional[List[float]]:
        """Use NavFoM planner to predict waypoints and convert to [vx, vy, wz].
            输入是当前RGB帧和指令文本，输出是机器人底盘的线速度vx、vy和角速度wz。内部流程包括：
            1) 将RGB帧编码为视觉特征tokens；
            2) 将当前帧的tokens加入历史，构建时空token输入；
            3) 调用planner模型预测未来轨迹；4) 从轨迹中提取第一步的位移并转换为速度指令。
        """
        ...
        try:
            ...
            # 结合文本指令并激活 LLM（模型前向推理），输出未来轨迹 (1, Mw, D)
            # Mw是预测的未来轨迹点数，D是每个点的维度（通常至少包含x,y位移）
            ...
            # Convert first waypoint to velocities using dt 转换第一步的位移到速度指令，假设规划的时间间隔为0.1秒
    ...
    def _render_frame_with_traj(self, rgb_frame_np: np.ndarray, traj_xyz: Optional[np.ndarray]) -> np.ndarray:
        """Overlay predicted trajectory onto the frame and return the composited RGB frame.
            功能：在当前RGB帧上叠加显示预测的轨迹点，便于可视化规划结果。输入是原始RGB帧和轨迹坐标，输出是叠加了轨迹的RGB帧。
        """
        try:
