...
class VisionCacheConfig:
    ...
    resize_mode: str = "letterbox"      # "letterbox" preserves aspect ratio; "stretch" is legacy.
    force_square_resize: bool = True    # deprecated legacy alias for exact stretch resize
    ...
    def __post_init__(self):
        resize_mode_env = os.getenv("VISION_RESIZE_MODE", "").strip().lower()
        if resize_mode_env:
            self.resize_mode = resize_mode_env
        if self.resize_mode == "square":
            self.resize_mode = "stretch"
        if self.resize_mode not in ("letterbox", "stretch"):
            raise ValueError(f"Unsupported resize_mode={self.resize_mode!r}; choose 'letterbox' or 'stretch'")
        ...
        if self.dino_model_name is None:
            ...
            if env_path and os.path.exists(env_path):
                ...
                local_fallback = "/data/hdt/ntv_data/weights/dinov3"
...
class VisionFeatureCacher(nn.Module):
    ...
    # Encoding helpers
    # ---------------------------------------
    def _resize_for_encoder(self, im: Image.Image) -> Image.Image:
    ...
    def _preprocess_pils(self, pil_list: List[Image.Image], which: str) -> Dict[str, torch.Tensor]:
        if self.cfg.force_square_resize:
            pil_list = [self._resize_for_encoder(im) for im in pil_list]
        if which == 'dino':
