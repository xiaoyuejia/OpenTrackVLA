...
EVAL_SCRIPT="${EVAL_SCRIPT:-eval_unrealzoo_multi_agent.py}"
...
EVAL_IMAGE_SIZE="${EVAL_IMAGE_SIZE:-384}"
EVAL_VISION_RESIZE_MODE="${EVAL_VISION_RESIZE_MODE:-letterbox}"
EVAL_FPS="${EVAL_FPS:-10}"
EVAL_DT="${EVAL_DT:-0.1}"
EVAL_HISTORY_FRAME_DT="${EVAL_HISTORY_FRAME_DT:-0.1}"
EVAL_WAYPOINT_SOURCE_DT="${EVAL_WAYPOINT_SOURCE_DT:-${EVAL_DT}}"
...
EVAL_HUMAN_WAYPOINT_STALL_WINDOW="${EVAL_HUMAN_WAYPOINT_STALL_WINDOW:-20}"
EVAL_HUMAN_WAYPOINT_STALL_DISTANCE="${EVAL_HUMAN_WAYPOINT_STALL_DISTANCE:-20.0}"
...
EVAL_INIT_FROM_RECORDED_AGENT_POSES="${EVAL_INIT_FROM_RECORDED_AGENT_POSES:-0}"
...
EVAL_ROBOTDOG_SPEED_GAIN="${EVAL_ROBOTDOG_SPEED_GAIN:-1.0}"
...
EVAL_ROBOTDOG_CAMERA_MOUNTS="${EVAL_ROBOTDOG_CAMERA_MOUNTS:-140:0:110,170:0:120,110:0:95,0:120:110,0:90:100,40:90:110,40:-90:110}"
...
EVAL_DRONE_IDEAL_FOLLOW_DIST="${EVAL_DRONE_IDEAL_FOLLOW_DIST:-4.25}"
...
EVAL_DRONE_VX_SCALE="${EVAL_DRONE_VX_SCALE:-1.0}"
EVAL_DRONE_VY_SCALE="${EVAL_DRONE_VY_SCALE:-1.0}"
...
for scene in scenes:
    if requested_episodes == "manifest":
        ...
        if episodes <= 0:
            print(f"[plan] skip {scene}: manifest test episodes={episodes}", file=sys.stderr)
            continue
    else:
...
run_eval_scene() {
    ...
    local eval_args=(
        ...
        --vision-resize-mode "${EVAL_VISION_RESIZE_MODE}"
        --fps "${EVAL_FPS}"
        --dt "${EVAL_DT}"
        --history-frame-dt "${EVAL_HISTORY_FRAME_DT}"
        --waypoint-source-dt "${EVAL_WAYPOINT_SOURCE_DT}"
        ...
        --human-waypoint-stall-window "${EVAL_HUMAN_WAYPOINT_STALL_WINDOW}"
        --human-waypoint-stall-distance "${EVAL_HUMAN_WAYPOINT_STALL_DISTANCE}"
...
echo "VISION image_size=${EVAL_IMAGE_SIZE} resize_mode=${EVAL_VISION_RESIZE_MODE}"
...
echo "CLOCK realtime=$((1-EVAL_DETERMINISTIC_STEP)) waypoint_timing=${EVAL_REALTIME_WAYPOINT_TIMING} control_dt=${EVAL_DT}s history_dt=${EVAL_HISTORY_FRAME_DT}s waypoint_source_dt=${EVAL_WAYPOINT_SOURCE_DT}s"
...
echo "INIT_FOLLOWERS_BEHIND_TARGET=${EVAL_INIT_FOLLOWERS_BEHIND_TARGET}"
